...
void main() {
    ...
    float specular = pow(clamp(dot(vNormal, halfLE), 0.0, 1.0),16.0);
    vec4 dest_color = texture2D(texture, vTexCoord) * vec4(vec3(diffuse), 1.0) + vec4(vec3(specular), 1.0) + ambientColor;
